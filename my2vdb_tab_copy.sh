...
echo "cat /root/tmp/conversions.txt | pigz -4 -p 6 |nc -l ${ncport} &"|${sshCmd} ${myHost} sudo bash
xsize=$(echo ls -la /root/tmp/conversions.txt |awk '{ print $5 }'|${sshCmd} ${myHost} sudo bash)
echo "== Piped COPY to VDB:"
echo "nc mm3.propellerads.com 8867|pv -pterab -s ${xsize} -f|/opt/vertica/bin/vsql -wYGtQaRSdGrVe1g -d openx -U dbadmin -c \"BEGIN; DELETE FROM openx.conversions; COPY openx.conversions FROM STDIN GZIP DELIMITER ',' NULL 'NULL' ENCLOSED BY '\"' DIRECT;\""
